...
describe('QueueProcessor', () => {
    ...
    it('should hydrate metadata for pending items', async () => {
        ...
        vi.mocked(downloadQueue.getPendingItems).mockReturnValue([
            mockItem as unknown as ReturnType<typeof downloadQueue.getPendingItems>[number]
        ]);
        const mockApi = (processor as unknown as { api: { getTrack: ReturnType<typeof vi.fn> } })
            .api;
        mockApi.getTrack.mockResolvedValue({
            success: true,
            data: {
                title: 'Real Title',
                performer: { name: 'Real Artist' },
    ...
    it('should activate circuit breaker after consecutive errors', async () => {
        ...
        const processPromise = (
            processor as unknown as { processNext: () => Promise<void> }
        ).processNext();
        ...
        expect(
            (processor as unknown as { consecutiveErrors: number }).consecutiveErrors
        ).toBeLessThan(5);
    });
